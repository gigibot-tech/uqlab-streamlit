...
while [ "$PROJECT_ROOT" != "/" ] && [ ! -f "$PROJECT_ROOT/pyproject.toml" ] && [ ! -f "$PROJECT_ROOT/uv.lock" ]; do
...
exec "$PYTHON" -m streamlit run "$PROJECT_ROOT/streamlit_app_progressive.py" --server.port "${FRONTEND_PORT}"
