...
APP_FILE="$PROJECT_ROOT/streamlit_app_progressive.py"
...
echo ""
echo -e "${YELLOW}▶ streamlit run $APP_FILE${NC}"
echo -e "${YELLOW}Press Ctrl+C to stop.${NC}"
echo ""

# python -m streamlit avoids broken #! shebangs if .venv was copied from elsewhere.
exec "$PYTHON" -m streamlit run "$APP_FILE" --server.port "${FRONTEND_PORT}"
