...
echo -e "${YELLOW}   PYTHONPATH=$REPO_ROOT/src${NC}"
echo ""
echo -e "${YELLOW}▶ streamlit run $REPO_ROOT/streamlit_app_progressive.py${NC}"
echo -e "${YELLOW}Press Ctrl+C to stop.${NC}"
echo ""

# python -m streamlit avoids broken #! shebangs if .venv was copied from elsewhere.
exec "$PYTHON" -m streamlit run "$REPO_ROOT/streamlit_app_progressive.py" --server.port "${FRONTEND_PORT}"
