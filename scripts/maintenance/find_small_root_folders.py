...
def suggest_relocation(stats: FolderStats, repo_root: Path) -> str:
    ...
    if name == "configs":
        return "candidate, but keep at root: experiment presets are user-facing and referenced by docs/notebooks; move only if configs_dir() and all hardcoded paths are updated"
    if name == "data":
    ...
    if "flask" in name or "api" in name:
        return "candidate: could fold into backend/ or src/ if it does not duplicate FastAPI functionality"
