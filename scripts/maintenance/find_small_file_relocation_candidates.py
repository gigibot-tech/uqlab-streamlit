...
def suggest_relocation(folder: Path, files: list[tuple[Path, int]]) -> list[str]:
    ...
    if folder.name == "configs":
        ...
        if any(
            str(f.relative_to(REPO_ROOT)).startswith("configs/test") for f, _ in files
        ):
            hints.append(
...
def print_report(results: dict[str, dict], threshold: int) -> None:
    ...
    if not candidates:
        print(
            "No root-level directory (with more than one countable file) has all files under the threshold."
        )
        return
    ...
    for name, data in sorted(results.items(), key=lambda kv: kv[1]["max_loc"]):
        ...
        marker = (
            "all under threshold"
            if data["all_under_threshold"]
            else f"max {data['max_loc']} LoC"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--threshold", type=int, default=300, help="LoC threshold (default: 300)"
    )
