...
Read **[`../UQLAB_FLOW.md`](../UQLAB_FLOW.md)** for experiments, `fit` / `predict_disentangling`, `signal_table`, and the paper benchmark. This page is run commands + folder map only.
...
|-----|--------|----------------|
| **Config** | [`../../src/uqlab_orchestrator/`](../../src/uqlab_orchestrator/) | `workflow` dict → nested YAML via [`../../src/uqlab_orchestrator/run_spec.py`](../../src/uqlab_orchestrator/run_spec.py) |
...
Wizard → YAML field mapping: [`../features/workflow-config.md`](../features/workflow-config.md)
...
- **Pipeline / IoC** — [`../../src/uqlab/runner/patterns.py`](../../src/uqlab/runner/patterns.py): stages own the flow, callers do not.
- **Factory** — [`../../src/uqlab/models/factory.py`](../../src/uqlab/models/factory.py): `build_model(config)`.
- **Strategy** — [`../../src/uqlab/shared/config/signals.py`](../../src/uqlab/shared/config/signals.py): `evaluation.signals` picks predictive / logit / attribution families.
...
- Hiding UI blocks → sidebar **UI debug**; see [`../../.cursor/skills/ui-debug/`](../../.cursor/skills/ui-debug/SKILL.md) and [`../features/ui-debug.md`](../features/ui-debug.md).
...
- Sweep grouping: [`../../src/uqlab_orchestrator/sweep_groups.py`](../../src/uqlab_orchestrator/sweep_groups.py)

## Do not read (unless debugging)

- `archive/` — old experiments
- Superseded root docs (`SYSTEM_FLOW.md`, `EXPERIMENT_FLOW.md`, `REGISTRIES.md`) redirect to `../UQLAB_FLOW.md`
