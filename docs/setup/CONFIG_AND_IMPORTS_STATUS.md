...
Located in [`src/uqlab_core/configs/`](../../src/uqlab_core/configs:1) (reachable via a `configs/` symlink at the repo root):
```
...
```
uqlab-streamlit/
├── src/uqlab_core/configs/     # ✅ YAML configs (still used; `configs/` symlink preserved)
│   ├── experiment/
│   └── test/
├── src/
...
### Config Strategy
- **Short term**: Keep using YAML configs in `src/uqlab_core/configs/` (reachable via `configs/` symlink)
