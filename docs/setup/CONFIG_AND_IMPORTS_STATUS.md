...
Located in [`src/configs/`](configs:1):
```
src/configs/
...
```
uqlab-streamlit/
├── src/configs/                    # ✅ YAML configs (still used)
│   ├── experiment/
│   └── test/
├── src/
...
- **YAML files** in [`src/configs/`](configs:1) - Used by `run_fast.py` and scripts
...
### Config Strategy
- **Short term**: Keep using YAML configs in `src/configs/`
