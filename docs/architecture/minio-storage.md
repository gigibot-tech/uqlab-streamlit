...
### 5. Startup Script ✅
**File**: `scripts/runners/start-with-minio.sh` (executable)
...
```bash
cd uqlab-streamlit
./scripts/runners/start-with-minio.sh
```
...
- [ ] Test script: `./scripts/runners/start-with-minio.sh` should start both services

## Files Created/Modified

### Created
1. `docker-compose.yml` - MinIO service configuration
2. `scripts/runners/start-with-minio.sh` - Convenience startup script
