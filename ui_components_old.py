...
def render_batch_results(
    ...
    # Construct batch directory path from batch_id
    ...
    if not batch_id_str:
        ...
        if os.path.exists(batch_dir):
            ...
            # Find all experiment directories - FIXED PATH
            experiments_dir = Path(batch_dir) / "experiments"
            exp_dirs = []  # Initialize to avoid unbound variable
            ...
            if signal_data:
                ...
                signal_tabs = st.tabs(["📈 All Signals", "🎯 By Signal Type", "📋 Data Table"])
                
                # Tab 1: All signals in one chart
                with signal_tabs[0]:
                    import plotly.graph_objects as go
                    ...
                    for signal_name, data in sorted(signal_data.items()):
                        ...
                        if epis_points:
                            fig_all.add_trace(go.Scatter(
                                ...
                                mode="lines+markers",
                                name=f"{signal_name} (epistemic)",
                                line=dict(width=2, dash='dot'),
                                marker=dict(size=6, symbol='diamond'),
                            ))
                    
                    fig_all.update_layout(
                        title=f"All Per-Signal AUROC vs {x_param}",
                        xaxis_title=x_param,
                        yaxis_title="AUROC",
                        yaxis=dict(range=[0, 1]),
                        height=700,
                    ...
                    st.caption(f"Showing {len(signal_data)} signals × 2 uncertainty types = {len(signal_data)*2} curves")
                ...
                with signal_tabs[1]:
                    ...
                    for category, signal_names in signal_categories.items():
                        ...
                        fig_cat.update_layout(
                            title=f"{category} AUROC vs {x_param}",
                            xaxis_title=x_param,
                            yaxis_title="AUROC",
                            yaxis=dict(range=[0, 1]),
                            height=500,
