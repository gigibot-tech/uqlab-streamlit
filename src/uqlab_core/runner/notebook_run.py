...
def default_four_region_runs(root: Any) -> list[dict[str, Any]]:
    ...
    The ``root`` argument is kept for backward compatibility with notebooks
    that pass ``ctx.root``; config paths are resolved via :func:`configs_dir`.
    """
    del root  # unused: config paths are resolved via configs_dir()
    return [
        {
            "name": "fashion_mlp",
