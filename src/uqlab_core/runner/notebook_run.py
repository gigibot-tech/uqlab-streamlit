...
def default_four_region_runs(root: Any | None = None) -> list[dict[str, Any]]:
    ...
    cfg_root = configs_dir() if root is None else Path(root) / "src" / "uqlab_core" / "configs"
    return [
        {
            "name": "fashion_mlp",
            "config_path": cfg_root / "experiment" / "four_region_fashion_mlp.yaml",
            "class_regions": deepcopy(DEFAULT_FOUR_REGION_PRESET),
        },
        {
            "name": "cifar_resnet",
            "config_path": cfg_root / "experiment" / "four_region_cifar_resnet.yaml",
            "class_regions": deepcopy(DEFAULT_FOUR_REGION_PRESET),
        },
    ]
