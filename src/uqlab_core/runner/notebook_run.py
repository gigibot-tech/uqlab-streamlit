...
def default_four_region_runs(root: Any) -> list[dict[str, Any]]:
    ...
    del root  # configs are now bundled with the package; kept for API compatibility
    cfg_dir = configs_dir()
    return [
        {
            "name": "fashion_mlp",
            "config_path": cfg_dir / "experiment" / "four_region_fashion_mlp.yaml",
            "class_regions": deepcopy(DEFAULT_FOUR_REGION_PRESET),
        },
        {
            "name": "cifar_resnet",
            "config_path": cfg_dir / "experiment" / "four_region_cifar_resnet.yaml",
            "class_regions": deepcopy(DEFAULT_FOUR_REGION_PRESET),
        },
    ]
